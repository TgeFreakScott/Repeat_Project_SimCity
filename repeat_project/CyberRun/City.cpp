...
double City::distributePool(double& pool, Tile& tile, double rate = 0.0)
{
	...
	float maxPop = tile.maxPopulation * (tile.tileVariation + 1);
...
void City::load(std::string cityName, std::map<std::string, Tile>& tileWorld)
{
	int width = 0;
	int height = 0;
	...
	this->map.load(cityName + "_map.dat", width, height, tileWorld);
	tileChanged();
		...
		if (tile.tileType == TileType::INDUSTRIAL)
		{
			...
			for (auto& tile2 : this->map.tiles)
			{
				if (tile2.regions[0] == tile.regions[0] && tile2.tileType == TileType::INDUSTRIAL)
				{
					...
					if (receivedResources >= tile.tileVariation + 1) break;
				}
			}
			/* Turn resources into goods */
			tile.storeProduce += (receivedResources + tile.production)*(tile.tileVariation + 1);
		}
	}
		...
		if (tile.tileType == TileType::COMMERCIAL)
		{
			...
			for (auto& tile2 : this->map.tiles)
			{
				if (tile2.regions[0] == tile.regions[0] &&
					...
					tile2.storeProduce > 0)
				{
					while (tile2.storeProduce > 0 && receivedGoods != tile.tileVariation + 1)
					{
						--tile2.storeProduce;
						++receivedGoods;
				...
				if (receivedGoods == tile.tileVariation + 1) break;
			}
