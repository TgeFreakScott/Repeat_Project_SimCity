...
void StateEditor::update(const float dt)
{
	...
	this->guiSystem.at("infoBar").setEntryText(4, tileToString(currentTile->tileType));
		...
		case sf::Event::MouseButtonPressed:
		{
			...
			else if (event.mouseButton.button == sf::Mouse::Left)
			{
				...
				if (this->guiSystem.at("rightClickMenu").visible == true)
				{
					...
					{
						this->currentTile = &this->game->TileWorld.at(msg);
					}
...
StateEditor::StateEditor(Game* game)
{
	...
	this->city = City("city", this->game->tileSize, this->game->TileWorld);
	...
	this->guiSystem.emplace("rightClickMenu", Gui(sf::Vector2f(196, 16), 2, false, this->game->stylesheets.at("button"),
		{
			std::make_pair("Flatten $" + this->game->TileWorld["grass"].getCost(), "grass"),
			std::make_pair("Forest $" + this->game->TileWorld["forest"].getCost(), "forest"),
			std::make_pair("Residential Zone $" + this->game->TileWorld["residential"].getCost(), "residential"),
			std::make_pair("Commercial Zone $" + this->game->TileWorld["commercial"].getCost(), "commercial"),
			std::make_pair("Industrial Zone $" + this->game->TileWorld["industrial"].getCost(), "industrial"),
			std::make_pair("Road $" + this->game->TileWorld["road"].getCost(), "road")
		}));
	...
	this->currentTile = &this->game->TileWorld.at("grass");
	this->actionState = ActionState::NONE;
