...
/*load map from disk*/
void Map::load(const std::string& filename, int width, int height, std::map<std::string, Tile>& tileWorld)
{
	...
	for (int pos = 0; pos < this->width * this->height; ++pos)
	{
		...
		switch (tileType)
		{
			...
			case TileType::GRASS:
				this->tiles.push_back(tileWorld.at("grass"));
				break;

			case TileType::FOREST:
				this->tiles.push_back(tileWorld.at("forest"));
				break;

			case TileType::WATER:
				this->tiles.push_back(tileWorld.at("water"));
				break;

			case TileType::RESIDENTIAL:
				this->tiles.push_back(tileWorld.at("residential"));
				break;

			case TileType::COMMERCIAL:
				this->tiles.push_back(tileWorld.at("commercial"));
				break;

			case TileType::INDUSTRIAL:
				this->tiles.push_back(tileWorld.at("industrial"));
				break;

			case TileType::ROAD:
				this->tiles.push_back(tileWorld.at("road"));
				break;
		}

		Tile& tile = this->tiles.back();
		inputFile.read((char*)&tile.tileVariation, sizeof(int));
		inputFile.read((char*)&tile.regions, sizeof(int) * 1);
		inputFile.read((char*)&tile.population, sizeof(double));
		inputFile.read((char*)&tile.storeProduce, sizeof(float));
	}
...
void Map::save(const std::string& filename)
{
	...
	for (auto tile : this->tiles)
	{
		...
		outputFile.write((char*)&tile.tileVariation, sizeof(int));
		outputFile.write((char*)&tile.regions, sizeof(int) * 1);
		outputFile.write((char*)&tile.population, sizeof(double));
		outputFile.write((char*)&tile.storeProduce, sizeof(float));
	}
...
void Map::updateDirection(TileType tileType)
{
	for (int y = 0; y < this->height; ++y)
	{
		for (int x = 0; x < this->width; ++x)
		{
			...
			bool adjacentTiles[4][4] = { {0, 0, 0}, {0, 0, 0},{0, 0, 0} };
